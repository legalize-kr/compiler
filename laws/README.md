...
각 법령 Markdown은 YAML frontmatter를 포함합니다.

- 기본 필드: `제목`, `법령MST`, `법령ID`, `법령구분`, `법령구분코드`, `소관부처`,
  `공포일자`, `공포번호`, `시행일자`, `법령분야`, `상태`, `출처`
- `첨부파일`: 별표/서식 다운로드 링크 목록입니다. 없으면 `[]`입니다.
  각 항목은 `별표번호`, `별표가지번호`, `별표구분`, `제목`을 포함하고,
  원본 형식 링크가 있으면 `파일링크`, PDF 링크가 있으면 `PDF링크`를 포함합니다.
- `원본제목`: 제목 정규화가 적용된 경우에만 원래 제목을 기록합니다.
