...
지원하는 자치법규 종류는 `조례`, `규칙`, `훈령`, `예규`, `고시`, `의회규칙`입니다. `기타`는 원천 분류가 불명확하므로 본문 트리에 쓰지 않고 실패/격리 목록으로 관리합니다.
...
`body_source: parsing-failed`인 문서는 국가법령정보센터 XML에 조문 또는 부칙 본문이 없고, 별표·첨부파일만 제공되거나 원천 XML에 본문이 비어 있는 경우입니다.
